...
export function createDemoState() {
  ...
  const crews = [
    ...
    { id: 'crew-2', name: 'Инженерная бригада', specialty: 'Электрика и инженерные сети', leadId: 'worker-4', memberIds: ['worker-4', 'worker-5'], phone: '', notes: '', active: true },
  ];
  const constructionStages = CONSTRUCTION_TEMPLATES.slice(0, 8).map((template, index) => ({ id: `stage-${index + 1}`, siteId: 'site-1', title: template.title, group: template.group, status: index < 2 ? 'done' : index === 2 ? 'doing' : index === 3 ? 'ready' : 'planned', progress: index < 2 ? 100 : index === 2 ? 45 : 0, plannedStart: offsetDate(index * 3), plannedFinish: offsetDate(index * 3 + template.duration), actualStart: index < 3 ? offsetDate(index * 3) : '', actualFinish: index < 2 ? offsetDate(index * 3 + template.duration - 1) : '', assigneeId: index === 2 ? 'worker-1' : '', crewId: index >= 2 && index <= 4 ? 'crew-1' : index === 7 ? 'crew-2' : '', dependencyIds: index ? [`stage-${index}`] : [], notes: '', blockReason: '', comments: [], attachments: [], createdAt: stamp, updatedAt: stamp }));
  const logistics = [{ id: 'route-1', number: 'Р-0001', siteId: 'site-1', orderId: 'order-1', plannedAt: dueAt(2, '09:00'), deliveryWindow: '09:00–12:00', vehicle: 'ГАЗель NEXT · А123ВС', driverId: 'worker-2', status: 'planned', carrier: 'Собственный транспорт', loadingAddress: 'Производство ЭФТ', unloadingAddress: sites[0].address, note: 'Домокомплект, крепёж и монтажный инструмент', createdAt: stamp, updatedAt: stamp }, { id: 'route-2', number: 'Р-0002', siteId: 'site-2', orderId: 'order-2', plannedAt: dueAt(4, '08:00'), deliveryWindow: '08:00–11:00', vehicle: 'Манипулятор · подрядчик', driverId: '', status: 'loading', carrier: 'ТК Север', loadingAddress: 'Производство ЭФТ', unloadingAddress: sites[1].address, note: 'Нужен пропуск на объект', createdAt: stamp, updatedAt: stamp }];
...
export function validateState(state) {
  ...
  if (state.sites.some((s) => !has('clients', s.clientId)) || state.leads.some((l) => !has('sites', l.siteId) || (l.ownerId && !has('employees', l.ownerId)) || !LEAD_STAGES.some((v) => v.id === l.status)) || state.orders.some((o) => !has('sites', o.siteId) || (o.leadId && !has('leads', o.leadId)))) throw new Error('Нарушены связи клиентов, заявок и заказов');
