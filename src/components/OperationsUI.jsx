...
export function Report({ report, onClose }) {
  const signatures=report.signatureLabels||['Составил','Проверил'];
  return createPortal(<Dialog title="Печатная форма" onClose={onClose} wide><div className={`report-sheet ${report.manual?'manual-form':''}`}><div className="report-brand"><img src="./eft-logo.jpg" alt="ЭФТ" /><span>ЭФТ · CRM · версия {RELEASE}</span></div><h2>{report.title}</h2><p>{report.subtitle}</p>{report.fields?.length?<div className="report-fields">{report.fields.map(([label,value],index)=><p key={index}><span>{label}</span><strong>{value||' '}</strong></p>)}</div>:null}<div className="table-wrap"><table><thead><tr>{report.headers.map((h, i) => <th key={i}>{h}</th>)}</tr></thead><tbody>{report.rows.map((row, i) => <tr key={i}>{row.map((value, j) => <td key={j}>{value}</td>)}</tr>)}</tbody></table></div>{report.footer ? <p className="report-footer">{report.footer}</p> : null}<p className="report-signature">{signatures.map((label,index)=><span key={label}>{label} ____________________{index<signatures.length-1?'　 ':''}</span>)}</p></div><footer className="dialog-actions"><button className="button" onClick={() => downloadCSV(report.title, report.headers, report.rows)}>Скачать CSV</button><button className="button primary" onClick={() => window.print()}>Печать / PDF</button></footer></Dialog>, document.body);
}
