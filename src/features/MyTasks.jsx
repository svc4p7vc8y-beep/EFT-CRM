...
function TaskDetails({ state, task, onOpen, onEdit, onReschedule }) {
  ...
  return <aside className="task-side"><div className="task-side-head"><div><h2>{task.title}</h2><span className={`priority ${task.priority}`}>{task.priority === 'high' ? 'Высокий приоритет' : 'Обычный приоритет'}</span></div><button className="text-link" onClick={() => onOpen(task.id)}>Открыть</button></div>
    <dl><dt>Ответственный</dt><dd><Avatar id={task.assigneeId} state={state} />{person?.name || 'Не назначен'}</dd><dt>Объект</dt><dd>{site?.name || 'Не привязан'}</dd>{stage ? <><dt>Этап</dt><dd>{stage.title}</dd></> : null}{crew ? <><dt>Бригада</dt><dd>{crew.name}</dd></> : null}<dt>Срок выполнения</dt><dd>{task.dueAt ? dateLabel(task.dueAt) : 'Не назначен'}</dd><dt>Статус</dt><dd><Badge value={task.status} stages={TASK_STAGES} /></dd></dl>
...
export function MyTasks({ state, search, command, onCreate, onOpen, onEdit, onMove }) {
  ...
  return <section className="page my-tasks-page"><header className="my-tasks-title"><div><h1>Мои задачи</h1><p>Общий список, свободные задачи, загрузка сотрудников и сроки выполнения</p></div><button className="button" onClick={onCreate}><Plus size={17} />Расширенная задача</button></header>
    ...
    <div className="task-content"><div className="task-main">
      ...
      {period === 'all' && view === 'list' ? <>{needsPlanning.length ? <section className="assignment-section"><h2><UserRoundX size={18} />Нужно распределить <span>{needsPlanning.length}</span></h2>{needsPlanning.map((task) => <TaskRow key={task.id} state={state} task={task} selected={selectedTask?.id === task.id} onSelect={setSelectedId} onOpen={onOpen} onMove={onMove} onReschedule={reschedule} />)}</section> : null}<section className="employee-tasks"><h2><ClipboardCheck size={18} />Запланированные задачи <span>{plannedOpen.length}</span></h2>{plannedOpen.map((task) => <TaskRow key={task.id} state={state} task={task} selected={selectedTask?.id === task.id} onSelect={setSelectedId} onOpen={onOpen} onMove={onMove} onReschedule={reschedule} />)}{!plannedOpen.length ? <Empty title="Запланированных задач нет" /> : null}</section></> : null}
      {period === 'day' && view === 'list' ? <>{unscheduled.length ? <section className="assignment-section"><h2><CalendarDays size={18} />Без срока — общий список <span>{unscheduled.length}</span></h2>{unscheduled.map((task) => <TaskRow key={task.id} state={state} task={task} selected={selectedTask?.id === task.id} onSelect={setSelectedId} onOpen={onOpen} onMove={onMove} onReschedule={reschedule} />)}</section> : null}{carried.length ? <section className="carry-section"><h2>Перенесено с прошлых дней <span>{carried.length}</span></h2>{carried.map((task) => <TaskRow key={task.id} state={state} task={task} carried selected={selectedTask?.id === task.id} onSelect={setSelectedId} onOpen={onOpen} onMove={onMove} onReschedule={reschedule} />)}</section> : null}{unassignedToday.length ? <section className="assignment-section"><h2><UserRoundX size={18} />Не назначены на дату <span>{unassignedToday.length}</span></h2>{unassignedToday.map((task) => <TaskRow key={task.id} state={state} task={task} selected={selectedTask?.id === task.id} onSelect={setSelectedId} onOpen={onOpen} onMove={onMove} onReschedule={reschedule} />)}</section> : null}{listGroups.map(({ person, tasks: rows }) => <section className="employee-tasks" key={person.id}><h2><Avatar id={person.id} state={state} />{person.name}<span>{rows.length}</span></h2>{rows.map((task) => <TaskRow key={task.id} state={state} task={task} selected={selectedTask?.id === task.id} onSelect={setSelectedId} onOpen={onOpen} onMove={onMove} onReschedule={reschedule} />)}</section>)}{!unscheduled.length && !carried.length && !unassignedToday.length && !listGroups.length ? <Empty title="На выбранную дату задач нет">Добавьте задачу сверху или выберите другую дату.</Empty> : null}</> : null}
    </div><TaskDetails state={state} task={selectedTask} onOpen={onOpen} onEdit={onEdit} onReschedule={reschedule} /></div>
  </section>;
}
