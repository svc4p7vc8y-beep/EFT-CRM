...
import { Field, PageHeading } from '../components/UI.jsx';
import { STAFF_ROLES, employee } from '../app/model.js';
...
export function EmployeeForm({ person, onSubmit, onClose }) {
  return <SettingsForm onClose={onClose} onSubmit={(data) => onSubmit({ name: data.get('name'), role: data.get('role'), department: data.get('department'), phone: data.get('phone'), email: data.get('email'), notes: data.get('notes'), active: data.has('active') })}>
...
export function StaffSettings({ state, focus, search, onEmployee, onCrew }) {
  ...
  return <section className="page staff-settings"><PageHeading title={focus === 'crews' ? 'Бригады' : 'Настройки'} description="Сотрудники, должности и состав рабочих бригад" />
    ...
    {section === 'employees' ? <><div className="settings-section-head"><div><h2>Сотрудники</h2><p>Менеджеры, цех и другой персонал</p></div><button className="button primary" onClick={() => onEmployee()}><Plus size={18} />Добавить сотрудника</button></div><div className="table-wrap"><table><thead><tr><th>Сотрудник</th><th>Должность</th><th>Подразделение</th><th>Контакт</th><th>Статус</th><th>Действие</th></tr></thead><tbody>{employees.map((person) => <tr key={person.id}><td><strong>{person.name}</strong></td><td>{person.role}</td><td>{person.department || '—'}</td><td>{person.phone || person.email || '—'}</td><td>{person.active ? 'Работает' : 'Неактивен'}</td><td><button className="text-link" onClick={() => onEmployee(person.id)}>Редактировать</button></td></tr>)}</tbody></table>{!employees.length ? <p className="settings-empty">Сотрудники не найдены.</p> : null}</div></> : null}
    {section === 'crews' ? <><div className="settings-section-head"><div><h2>Рабочие бригады</h2><p>Состав, специализация и ответственный</p></div><button className="button primary" onClick={() => onCrew()}><Plus size={18} />Добавить бригаду</button></div><div className="crew-grid">{crews.map((crew) => <article className="crew-card" key={crew.id}><div className="crew-card-top"><span className="crew-icon"><UsersRound size={20} /></span><span>{crew.active ? 'Действует' : 'Неактивна'}</span></div><h3>{crew.name}</h3><p>{crew.specialty || 'Специализация не указана'}</p><dl><dt>Бригадир</dt><dd>{employee(crew.leadId, state)?.name || 'Не назначен'}</dd><dt>Состав</dt><dd>{crew.memberIds.length} чел.</dd></dl><button className="text-link" onClick={() => onCrew(crew.id)}>Редактировать бригаду</button></article>)}{!crews.length ? <div className="settings-empty">Бригад пока нет. Добавьте первую бригаду и выберите сотрудников.</div> : null}</div></> : null}
